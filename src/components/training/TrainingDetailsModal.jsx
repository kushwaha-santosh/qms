...
export default function TrainingDetailsModal({ open, record, onClose }) {
  ...
  const trainer = r.trainer
    ...
    : "—";
  const rows = [
    ["Training Number", r.trainingNumber],
    ["Title", r.title],
    ["Training Type", p(r.trainingType)],
    ["Department", p(r.department)],
    ["Trainer", trainer],
    [
      "Participants",
    ...
    ["Scheduled Date", fmt(r.scheduledDate)],
    ["Due Date", fmt(r.dueDate)],
    ["Completion Date", fmt(r.completionDate)],
    ["Status", p(r.status)],
