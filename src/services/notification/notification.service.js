...
const userScope = (user) => {
  const userId = oid(user?._id || user?.id);
  if (!userId) throw error("Authenticated user is required.", 401);
  return { userId };
};
const orgId = (user) => oid(user?.organizationId?._id || user?.organizationId);
const serialize = (item) => {
  if (!item) return null;
  const n = item.toObject ? item.toObject() : item;
  return {
    ...n,
    _id: id(n._id),
    userId: id(n.userId),
    organizationId: id(n.organizationId),
    recordId: id(n.recordId),
  };
};
  ...
  try {
    ...
    const targetOrg =
      organizationId !== undefined
        ? oid(organizationId?._id || organizationId)
        : orgId(user);
    if (dedupeKey) {
    ...
    const item = await Notification.create({
      ...
      recordId,
      recordNumber: recordNumber ? String(recordNumber).trim() : null,
...
export const createNotifications = async (args = {}) => {
  const recipients = [
    ...new Set(
      (Array.isArray(args.userIds) ? args.userIds : [])
        .filter(Boolean)
        .map(String),
    ),
  ];
    ...
    await Promise.all(
      recipients.map((userId) =>
        createNotification({
          ...args,
          userId,
          dedupeKey: args.dedupeKey ? `${args.dedupeKey}:${userId}` : null,
        }),
      ),
    )
  ).filter(Boolean);
...
export const listNotifications = async ({
  ...
  const l = Math.min(Math.max(Number.parseInt(limit, 10) || 20, 1), 100);
  const active = {
  ...
  const [items, total, unreadCount] = await Promise.all([
    ...
    Notification.countDocuments({ ...scope, isRead: false, ...active }),
  ]);
  return {
...
export const markNotificationRead = async ({ user, notificationId } = {}) =>
  updateRead(user, notificationId, true);
export const markNotificationUnread = async ({ user, notificationId } = {}) =>
  updateRead(user, notificationId, false);
const updateRead = async (user, notificationId, isRead) => {
  const n = oid(notificationId);
  if (!n) throw error("Invalid notification ID.");
  const item = await Notification.findOneAndUpdate(
    { ...userScope(user), _id: n },
    { $set: { isRead, readAt: isRead ? new Date() : null } },
    { new: true },
  ).lean();
  if (!item) throw error("Notification not found.", 404);
  return serialize(item);
};

export const markAllNotificationsRead = async ({ user } = {}) => {
  const r = await Notification.updateMany(
    { ...userScope(user), isRead: false },
...
export const deleteNotification = async ({ user, notificationId } = {}) => {
  const n = oid(notificationId);
  if (!n) throw error("Invalid notification ID.");
  const r = await Notification.deleteOne({ ...userScope(user), _id: n });
  if (!r.deletedCount) throw error("Notification not found.", 404);
  return { deleted: true };
};
